...
public class ErrorReport extends LinkedMap implements Serializable
{
     ...
     */
    public void addError(String fieldName, String message, String suggestedValue, ValidationContext vcontext) {
        ValidationEvent vevent = null;
        if (vcontext.getValidationEventFactory() != null) {
            vevent = vcontext.getValidationEventFactory().create(fieldName, message, suggestedValue);
        } else {
            LoggerFactory.getLogger(DATAX.VALIDATION_LOGNAME).warn("there is no validation event factory set,check you configuration");
        }
        if (vevent != null) {
